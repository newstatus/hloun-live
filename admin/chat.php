...
############################
if($chat->login()==false){
 $inclued=1; include 'login.php'; exit(); die();
 }
if(!isset($_GET['id']) or $_GET['id']==''){
 echo '<h1>Sorry Please Add room Id</h1>'; exit(); die();
 } 
$romdid = abs(intval($_GET['id']));
if($chat->SQL("select * from rooms where id='$romdid'","num") <=0) { 
echo 'Sorry Room Not Found !'; exit(); die(); } 
?>
<!DOCTYPE html>
<html>
<head>
...
function add(value){
		  var data = {"msg":value,"roomid":"<?=abs(intval($_GET['id']))?>"} 
			$.ajax({
						type:"POST",
 ...
 function getnew(){
    	 var lastid =  $('.msgs:last-child').attr("msgid");
		  var data = {"lastid":lastid,"roomid":"<?=abs(intval($_GET['id']))?>"} 
		  $.ajax({
						type:"POST",
...
<header>
	...
	<div class="container-fluid">
		...
		<div id="messages">
			...
			<?php echo $chat->getnewmsgs($romdid,1); ?>	  
			</div>
		</div>
		<br>
		<div class="row-fluid">
